...
const SupplyForm = ({ event, client, currentUser, data, closeModal }) => {
    ...
    const formButton=()=>{
        ...
        if(exists.length>0){
            ...
            return(<button onClick={(event)=>{volunteerSubmit(event);}}>Bring Item</button>)
        }
    ...
    if(currentUser.id){
       return (<div className="modal-form">
        ...
        <form className="modal-event" >
            <p>Donate Supplies</p>
            <div className="modal-event-field"><label>Quantity</label><input type="number" name="quantity" onChange={onChange} required /></div>
            {formButton()}
        </form>
    </div>);
    }else{
